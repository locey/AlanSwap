...
export default function ConnectWalletButton() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setChainId(parseInt(cId, 16) || null);
    } catch (e) {
      // no-op
    }
  }, []);
  ...
  const connect = async () => {
    setErr(null);
    if (!hasWindowEth()) {
      setErr('未检测到钱包，请安装 MetaMask 或使用内置钱包。');
      return;
    }
    try {
      setBusy(true);
      ...
      setAddress(addr);
      setChainId((parseInt(cId, 16) || null));
      console.log('connect_wallet_btn: ', { addr, cId });
  ...
  async function fetchNonce(address) {
    try {
      const url = `${VITE_BASEURL}/api/v1/auth/nonce/?address=${address}`;
  ...
  async function verifySignature({ nonce, address, signature }) {
    try {
      const res = await fetch(
        `${VITE_BASEURL}/api/v1/auth/verify`,
        {
          method: 'POST',
    ...
    <div className={`flex items-center gap-2`}>
      {!walletConnected ? (
        <button
          onClick={connect}
          disabled={busy}
          className={`font-semibold px-2 py-1 rounded-2xl transition-all duration-200 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 hover:-translate-y-1`}
        >
          连接钱包
        </button>
      ) : (
        <div className="flex items-center gap-2">
          {address && (
            <span className="bg-green-500/20 text-green-400" title={address}>
              {truncate(address)}
            </span>
          )}
          <button
            onClick={disconnect}
            disabled={busy}
            className="px-3 py-1 rounded-xl bg-zinc-200 dark:bg-zinc-700 text-sm hover:opacity-90 disabled:opacity-50 text-red-400"
          >
            断开
          </button>
          {chainId && (
            <span className="bg-green-500/20 text-green-400" title={chainId}>
              {chainId}
            </span>
          )}
        </div>
